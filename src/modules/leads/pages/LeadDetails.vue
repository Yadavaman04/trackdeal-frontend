<template>
  <div v-if="isLoading" class="animate-pulse space-y-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
      <div
        class="lg:col-span-8 h-96 bg-slate-200 dark:bg-slate-850 rounded-xl"
      ></div>
      <div
        class="lg:col-span-4 h-96 bg-slate-200 dark:bg-slate-850 rounded-xl"
      ></div>
    </div>
  </div>

  <div v-else-if="isError" class="section-panel p-8 text-center" role="alert">
    <h1 class="text-lg font-semibold">Unable to load this lead</h1>
    <p class="mt-2 text-body-sm text-text-secondary">
      Please try again to load the latest details.
    </p>
    <button type="button" class="btn-md btn-secondary mt-4" @click="refetch()">
      Try again
    </button>
  </div>
  ...
  <div v-else class="lead-workspace mx-auto max-w-[1440px] space-y-6">
    <LeadSummary
      :lead="lead"
      @activity="isActivityCenterOpen = true"
      @deal="isWonOpen = true"
    />
    ...
    >
      <div class="flex items-center gap-3">
        <div
          class="w-10 h-10 rounded-lg bg-amber-100 dark:bg-amber-900/50 text-amber-600 dark:text-amber-400 flex items-center justify-center text-xl shrink-0"
        >
        ...
        <div>
          <div class="flex items-center gap-2">
            <h4
              class="font-heading font-bold text-amber-900 dark:text-amber-200 text-sm"
            >
              Transferred Lead
            </h4>
            <span
              class="px-2 py-0.2 text-[9px] font-bold rounded bg-amber-200 dark:bg-amber-800 text-amber-900 dark:text-amber-100 uppercase"
              >External Partner</span
            >
          </div>
          <p class="text-xs text-amber-700 dark:text-amber-300 mt-0.5">
            Received from
            <strong class="font-semibold text-amber-950 dark:text-amber-150">{{
              lead.receivedFrom?.organizationName || "Partner Broker"
            }}</strong>
            <span v-if="lead.receivedFrom?.transferredByName">
              (Transferred by {{ lead.receivedFrom.transferredByName }})</span
            >
            <span
              v-if="lead.receivedFrom?.remarks"
              class="italic ml-2 text-slate-500"
              >— "{{ lead.receivedFrom.remarks }}"</span
            >
          </p>
        </div>
      </div>
      <div
        class="hidden sm:flex items-center gap-1.5 text-xs text-amber-700 dark:text-amber-400 font-semibold bg-amber-100/60 dark:bg-amber-900/30 px-3 py-1.5 rounded-lg border border-amber-200/50"
      >
    ...
    <div class="grid grid-cols-1 xl:grid-cols-12 gap-5 items-start">
      ...
      <div class="xl:col-span-8 min-w-0 space-y-5">
        <LeadProfile :lead="lead" @edit="isEditOpen = true" />

        <!-- Transferred Channel Partners & Brokers Card -->
        <div
          class="bg-surface border border-default rounded-xl p-4 shadow-none space-y-3"
        >
          <div
            class="flex flex-wrap items-center justify-between gap-3 border-b border-default pb-3"
          >
            <h4
              class="font-heading text-xs font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1.5"
            >
              <span class="inline-flex items-center gap-1.5"
                ><AppIcon name="buildings" :size="14" /> Channel partners</span
              >
              <span
                v-if="transferredCPsCount > 0"
                class="px-2 py-0.2 rounded-full text-[10px] bg-accent-100 text-accent-700 font-bold"
              >
            ...
            <button
              ...
              class="btn-sm btn-ghost text-accent-600"
            >
              + Transfer Lead
            </button>
          </div>
          ...
          <div
            v-if="transferredCPsList.length === 0"
            class="text-caption text-slate-400 text-center py-3"
          >
            No partners yet. Transfer this lead to collaborate with your
            network.
          </div>
          ...
          >
            ...
            >
              <div class="flex justify-between items-start">
                ...
                <span
                  class="px-1.5 py-0.5 rounded text-[9px] font-semibold bg-emerald-50 text-emerald-700 border border-emerald-200 shrink-0"
                >
                  Active
                </span>
              </div>
              <p
                class="text-caption text-slate-600 dark:text-slate-300 font-medium"
              >
                <AppIcon name="buildings" :size="12" class="inline" />
                {{ cp.officeName || "CP" }}
              </p>
              <p class="text-micro text-slate-400">
                <AppIcon name="phone" :size="11" class="inline" />
                {{ cp.phone || "N/A" }} •
                <AppIcon name="mapPin" :size="11" class="inline" />
                {{ cp.city || "CP" }}
              </p>
            </div>
          </div>
        ...
        <details
          class="lead-composer bg-surface border border-default rounded-xl p-4 space-y-4"
          v-if="lead.status !== 'won'"
        >
          <summary
            class="flex items-center justify-between text-body-sm font-semibold cursor-pointer"
          >
            Log a conversation or note <AppIcon name="add" :size="16" />
          </summary>
          ...
          <div v-if="activeLogTab === 'activity'" class="space-y-3 text-xs">
            <div class="grid grid-cols-2 gap-3">
              <select
                ...
                v-model="logType"
                class="bg-surface border border-default rounded-lg px-3 py-1.5 outline-none font-medium"
              >
              ...
              <input
                ...
                v-model="logOutcome"
                type="text"
                placeholder="Outcome (e.g. connected, busy...)"
                class="bg-surface border border-default rounded-lg px-3 py-1.5 outline-none"
              />
            </div>
            <textarea
              ...
              rows="2"
              placeholder="Summary notes of call or meeting..."
              class="w-full bg-surface border border-default rounded-lg px-3 py-1.5 outline-none resize-none"
            ></textarea>
            <button
              @click="submitActivityLog"
              class="bg-primary hover:bg-opacity-90 text-white font-bold px-4 py-1.5 rounded-lg"
            >
              Log Interaction
            </button>
          </div>
        ...
        <div
          class="bg-surface border border-default rounded-xl overflow-hidden shadow-none flex flex-col"
        >
          <div
          ...
          <div class="p-4 flex-1">
            ...
            <div v-if="activeWorkspaceTab === 'overview'" class="space-y-4">
              <FollowUpPanel
                ...
                @schedule="
                  ...
                  isSchedulingOutreach = true;
                "
              />
              <div class="border border-default rounded-xl p-4 bg-slate-50/50">
                <h5
                  class="text-xs font-bold text-slate-800 dark:text-slate-200 mb-2"
                >
                  Requirements Notes
                </h5>
                <p class="text-xs text-slate-600 leading-relaxed">
                  {{ lead.requirements?.notes || "No notes specified." }}
                </p>
              </div>
            </div>
            ...
            <!-- 2. ACTIVITIES TAB -->
            <div
              v-else-if="activeWorkspaceTab === 'activities'"
              class="space-y-3"
            >
              <div
                v-for="act in loggedActivities"
                :key="act.id || act._id"
                class="p-3 border border-default rounded-xl bg-slate-50/40 text-xs flex items-center justify-between"
              >
                <div>
                  <span
                    class="font-bold capitalize text-slate-800 dark:text-slate-200"
                    >{{ act.type }} outreach</span
                  >
              ...
              <div
                v-if="loggedActivities.length === 0"
            ...
            <!-- 4. SITE VISITS TAB -->
            <div
              v-else-if="activeWorkspaceTab === 'site_visits'"
              class="space-y-3"
            >
              <div
              ...
              >
                ...
                <span
                  class="px-2 py-0.5 rounded text-[8px] font-bold uppercase bg-blue-50 text-blue-700"
                >
                  {{ visit.status }}
                </span>
              </div>
            ...
            <div v-else-if="activeWorkspaceTab === 'deals'" class="space-y-3">
              <div
                v-if="lead.status === 'won'"
                class="p-4 border border-default rounded-xl bg-emerald-50/20 text-xs flex items-center justify-between"
              >
                <div>
                  <h5 class="font-bold text-slate-800">
                    Deal #DL-88301 (Initiated)
                  </h5>
                  <p class="text-[10px] text-slate-400 mt-0.5">
                    Conversion deal created on won transition.
                  </p>
                </div>
                <router-link
                  to="/app/deals"
                  class="text-xs text-primary font-bold hover:underline inline-flex items-center gap-1"
                  >View deal <AppIcon name="arrowRight" :size="12"
                /></router-link>
              </div>
            ...
            <!-- 6. DOCUMENTS TAB -->
            <div
              v-else-if="activeWorkspaceTab === 'documents'"
              class="grid grid-cols-2 gap-3 text-xs"
            >
              <div
                v-for="doc in documents"
                :key="doc.id"
                class="p-2.5 border border-default rounded-lg bg-slate-50/50 flex items-center justify-between"
              >
                <div>
                  <span
                    class="font-bold text-slate-700 truncate block max-w-[120px]"
                    >{{ doc.name }}</span
                  >
              ...
              <div
                ...
                class="col-span-2 text-center py-8 text-slate-400"
              >
...
import LeadSummary from "../components/LeadSummary.vue";
...
// Load lead details
const { data: leadData, isLoading, isError, refetch } = useLeadQuery(leadId);
const lead = computed(() => leadData.value?.data || null);

// Customer Profile Modal & Linked Leads
const isCustomerModalOpen = ref(false);
const customerId = computed(
  () => lead.value?.customerId?._id || lead.value?.customerId || null,
);
...
const { data: tasksData, refetch: refetchTasks } = useQuery({
  ...
  queryFn: () =>
    apiClient
      .get("/tasks", { params: { leadId: leadId.value } })
      .then((res) => res.data),
  enabled: !!leadId,
});
