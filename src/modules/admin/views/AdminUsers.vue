      ...
      >
        <div class="bg-slate-900 border border-slate-800 rounded-2xl max-w-lg w-full p-6 shadow-2xl space-y-4">
          <div class="flex items-center justify-between border-b border-slate-800 pb-3">
            ...
            <button @click="showCreateModal = false" class="text-slate-400 hover:text-white p-1">✕</button>
          </div>
...
const editForm = ref({ firstName: '', lastName: '', role: 'org_admin', status: 'active', password: '' });
...
async function executeMoveUser() {
  saving.value = true;
  try {
    await moveAdminUserOrganization(selectedUser.value._id, targetMoveOrgId.value);
